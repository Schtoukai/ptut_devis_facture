...
namespace Logiciel_Devis_Facture.packModele
{
    public class Client
    {
        ...
        *                                                      */
        ...
        {
            string connstring = @"server=localhost;user id=root;password=R@a[i?G++{iPynQ;database=invoiceDatabase"; // tant que la bd n'est pas crée on ne la précise pas
            MySqlConnection connection;
            string Querry = "SELECT * FROM customer WHERE LOWER(nameCustomer) LIKE " + "'%" + request + "%'" + " OR " + "LOWER(phone) LIKE " + "'%" + request + "%';";
            Console.WriteLine(Querry);
            connection = new MySqlConnection(connstring);
            try
            {
